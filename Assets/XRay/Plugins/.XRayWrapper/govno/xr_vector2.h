...
template<typename T> struct _vector2 {
	...
	_vector2<T>&	_min(const _vector2<T>& a);
	_vector2<T>&	_max(const _vector2<T>& a);
...
template<typename T> inline _vector2<T>& _vector2<T>::_min(const _vector2<T>& a)
{
	if (x >= a.x)
		x = a.x;
	if (y >= a.y)
...
template<typename T> inline _vector2<T>& _vector2<T>::_max(const _vector2<T>& a)
{
	if (x < a.x)
		x = a.x;
	if (y < a.y)
