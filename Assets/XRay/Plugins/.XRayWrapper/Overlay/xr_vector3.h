...
namespace xray_re {
	...
	template<typename T> struct _vector3 {
		...
		_vector3<T>& _min(const _vector3<T>& v);
		_vector3<T>& _max(const _vector3<T>& v);
	...
	template<typename T> inline _vector3<T>& _vector3<T>::_min(const _vector3<T>& v)
	{
		if (x >= v.x)
			x = v.x;
		if (y >= v.y)
	...
	template<typename T> inline _vector3<T>& _vector3<T>::_max(const _vector3<T>& v)
	{
		if (x < v.x)
			x = v.x;
		if (y < v.y)
